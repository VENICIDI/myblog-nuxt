...
const messages = ref<ChatMessage[]>([
  {
    role: 'assistant',
    content: '你好！我是 DeepSeek，有什么我可以帮助你的吗？'
  }
])
...
function sendMessage() {
  ...
  currentStreamController.value = chatStream.stream(
    ...
    (error) => {
      ...
      // 只在内容为空时显示错误信息，避免覆盖已接收的内容
      if (!messages.value[aiMessageIndex].content.trim()) {
        messages.value[aiMessageIndex].content = `抱歉，我遇到了一点问题: ${error.message || '未知错误'}`;
      } else {
        // 如果已有内容，只在末尾添加小的提示
...
function cancelCurrentResponse() {
  if (currentStreamController.value && loading.value) {
...
function createNewChat() {
  ...
  if (messages.value.length > 1) {
    if (!confirm('确定要开始新对话吗？当前对话将不会保存。')) {
      return;
    }
  }
            ...
            <div class="chat-container" ref="chatContainerRef">
                <div v-if="messages.length === 1" class="welcome-section">
                    <div class="title">
                        ...
                        <h1 class="greeting">我是 DeepSeek，很高兴见到你！</h1>
                    </div>
                    <div class="description">我可以帮你写代码、读文件、写作各种创意内容，请把你的任务交给我吧~</div>
                </div>
                
                <div class="message-list">
                    <div v-for="(message, index) in messages" :key="index" 
                         :class="['message', message.role === 'assistant' ? 'message-ai' : 'message-user']">
                        <div class="message-avatar">
                            <template v-if="message.role === 'assistant'">
                                <svg viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M27.501 8.46875C27.249 8.3457 27.1406 8.58008 26.9932 8.69922C26.9434 8.73828 26.9004 8.78906 26.8584 8.83398C26.4902 9.22852 26.0605 9.48633 25.5 9.45508C24.6787 9.41016 23.9785 9.66797 23.3594 10.2969C23.2275 9.52148 22.79 9.05859 22.125 8.76172C21.7764 8.60742 21.4238 8.45312 21.1807 8.11719C21.0098 7.87891 20.9639 7.61328 20.8779 7.35156C20.8242 7.19336 20.7695 7.03125 20.5879 7.00391C20.3906 6.97266 20.3135 7.13867 20.2363 7.27734C19.9258 7.84375 19.8066 8.46875 19.8174 9.10156C19.8447 10.5234 20.4453 11.6562 21.6367 12.4629C21.7725 12.5547 21.8076 12.6484 21.7646 12.7832C21.6836 13.0605 21.5869 13.3301 21.501 13.6074C21.4473 13.7852 21.3662 13.8242 21.1768 13.7461C20.5225 13.4727 19.957 13.0684 19.458 12.5781C18.6104 11.7578 17.8438 10.8516 16.8877 10.1426C16.6631 9.97656 16.4395 9.82227 16.207 9.67578C15.2314 8.72656 16.335 7.94727 16.5898 7.85547C16.8574 7.75977 16.6826 7.42773 15.8193 7.43164C14.957 7.43555 14.167 7.72461 13.1611 8.10938C13.0137 8.16797 12.8594 8.21094 12.7002 8.24414C11.7871 8.07227 10.8389 8.0332 9.84766 8.14453C7.98242 8.35352 6.49219 9.23633 5.39648 10.7441C4.08105 12.5547 3.77148 14.6133 4.15039 16.7617C4.54883 19.0234 5.70215 20.8984 7.47559 22.3633C9.31348 23.8809 11.4307 24.625 13.8457 24.4824C15.3125 24.3984 16.9463 24.2012 18.7881 22.6406C19.2529 22.8711 19.7402 22.9629 20.5498 23.0332C21.1729 23.0918 21.7725 23.002 22.2373 22.9062C22.9648 22.752 22.9141 22.0781 22.6514 21.9531C20.5186 20.959 20.9863 21.3633 20.5605 21.0371C21.6445 19.752 23.2783 18.418 23.917 14.0977C23.9668 13.7539 23.9238 13.5391 23.917 13.2598C23.9131 13.0918 23.9512 13.0254 24.1445 13.0059C24.6787 12.9453 25.1973 12.7988 25.6738 12.5352C27.0557 11.7793 27.6123 10.5391 27.7441 9.05078C27.7637 8.82422 27.7402 8.58789 27.501 8.46875Z" fill="#4D6BFE"></path>
                                </svg>
                            </template>
                            <template v-else>
